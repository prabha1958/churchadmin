...
export default function EventsPage() {
    ...
    const openEdit = (ann: Event) => {
        setSelectedEvent(ann);
        setEditForm({
            ...ann,
            date: toDateInput(ann.date_of_event),
    ...
    const handleEditSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/admin/events/${selectedEvent.id}`,
                {
    ...
    const handlHideSubmit = async (id: any) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/admin/events/${id}/hide`, {
                method: "PATCH",
                headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: "application/json",
    ...
    const handleShowSubmit = async (id: any) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/admin/events/${id}/show`, {
                method: "PATCH",
                headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: "application/json",
        ...
        <div className="p-6 bg-base-100">
            ...
            <table className="table table-zebra w-full  text-center">
                ...
                <tbody className="text-blue-950">
                    {events?.map((ev) => (
                        <tr key={ev.id} className="odd:bg-blue-200 even:bg-blue-50">
                            <td>{ev.id}</td>
                            ...
                            <td>
                                <Button size="sm" onClick={() => openEdit(ev)} className="bg-amber-400 text-amber-950 font-bold my-1">
                                    Edit
                                </Button>
                            </td>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <Input
                                        required
                                        onChange={(e) =>
                                            setAddForm({ ...addForm, name_of_event: e.target.value })
                                        }
                                    />
                                </div>
                                <div>
                    ...
                    <form onSubmit={handleEditSubmit} className="relative">
                        ...
                        <div className="max-h-[65vh] overflow-y-auto px-6 py-4 space-y-6">
                            <div className="flex flex-col items-center space-y-4">
                                ...
                                <div className="flex flex-col space-y-0.5 w-full">
                                    <label className="text-amber-400">Name of event</label>
                                    <Input
                                        placeholder="Title"
                                        value={editForm.name_of_event || ""}
                                        onChange={(e) => setEditForm({ ...editForm, name_of_event: e.target.value })}
                                    />
                                </div>
